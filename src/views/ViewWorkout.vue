  ...
  <div class="max-w-screen-sm mx-auto px-4 py-10">
    ...
    <div v-if="dataLoaded">
      ...
      <div class="flex flex-col items-center p-8 rounded-md shadow-md bg-light-grey relative">
        ...
        <!-- Cardio Workout IMG-->
        <img v-if="data.workoutType === 'cardio'" src="@/assets/images/running-light-green.png" class="h-24 w-auto"
             alt="cardio">
        <!-- Strength Workout IMG-->
        <img v-else src="@/assets/images/dumbbell-light-green.png" class="h-24 w-auto"
             alt="strength">
        <span class="mt-6 py-1.5 px-5 text-xs text-white bg-at-light-green rounded-lg shadow-md"> {{ data.workoutType }}
        </span>
      ...
      <div class="mt-10 p-8 rounded-md flex flex-col items-center bg-light-grey shadow-md">
        ...
        <div v-if="data.workoutType === 'strength'" class="flex flex-col gap-y-4 w-full">
          <div class="flex flex-col gap-x-6 gap-y-2 relative sm:flex-row" v-for="(item, index) in data.excercises"
               :key="index">
            <div class="flex flex-2 flex-col md:w-1/3">
              <label for="exercise-name" class="mb-1 text-sm text-at-light-green">Exercise</label>
              <input id="exercise-name" type="text" v-if="edit" class="p-2 w-full text-gray-500 focus:outline-none"
          ...
          <button v-if="edit" @click="addExercise" type="button"
                  class="mt-6 py-2 px-6 rounded-sm text-sm self-start text-white bg-at-green duration-200 border-solid border-2 border-transparent hover:border-at-light-green hover:bg-white hover:text-at-light-green">
            Add Exercise
          </button>
        </div>
        ...
        <div v-else class="flex flex-col gap-y-4 w-full">
          <div class="flex flex-col gap-x-6 gap-y-2 relative sm:flex-row" v-for="(item, index) in data.excercises"
            ...
            <div class="flex flex-2 flex-col md:w-1/3">
              ...
              <select id="cardioType" v-if="edit" class="p-2 w-full text-gray-500 focus:outline-none"
            ...
            <img @click="deleteExercise(item.id)" v-if="edit" class="absolute h-4 w-auto -left-5 cursor-pointer"
                 src="@/assets/images/trash-light-green.png" alt="">
          </div>
          <button v-if="edit" @click="addExercise" type="button"
                  class="mt-6 py-2 px-6 rounded-sm text-sm self-start text-white bg-at-green duration-200 border-solid border-2 border-transparent hover:border-at-light-green hover:bg-white hover:text-at-light-green">
      ...
      <!-- Update -->
      <button @click="update" v-if="edit" type="submit"
              class="mt-6 py-2 px-6 rounded-sm text-sm self-start text-white bg-at-green duration-200 border-solid border-2 border-transparent hover:border-at-light-green hover:bg-white hover:text-at-light-green">
        Update Workout
      </button>
    </div>
...
export default {
  ...
  setup() {
    ...
    const deleteExercise = (id) => {
      if (data.value.excercises.length > 1) {
        data.value.excercises = data.value.excercises.filter((exercise) => exercise.id !== id);
        return;
      }
    ...
    const update = async () => {
      try {
        const {error} = await supabase.from('workouts').update({
          ...
          excercises: data.value.excercises,
        }).eq('id', currentId);
        if(error) throw error;
        edit.value = false;
