        ...
        <div v-if="workoutType === 'strength'" class="flex flex-col gap-y-4">
          <div class="flex flex-col gap-x-6 gap-y-2 relative md:flex-row" v-for="(item, index) in exercises"
            ...
            <div class="flex flex-col md:w-1/3">
              ...
              <input type="text" required class="w-full p-2 text-gray-500 focus:outline-none" id="exercise-name"
                     v-model="item.exercise">
            </div>
            <div class="flex flex-col flex-1">
              <label for="exercise-sets" class="mb-1 text-sm text-at-light-green">Sets</label>
              <input type="text" required class="w-full p-2 text-gray-500 focus:outline-none" id="exercise-sets"
                     v-model="item.sets">
            </div>
            <div class="flex flex-col flex-1">
              <label for="exercise-reps" class="mb-1 text-sm text-at-light-green">Reps</label>
              <input type="text" required class="w-full p-2 text-gray-500 focus:outline-none" id="exercise-reps"
                     v-model="item.reps">
            </div>
            <div class="flex flex-col flex-1">
              <label for="exercise-weight" class="mb-1 text-sm text-at-light-green">Weight (Kg's)</label>
              <input type="text" required class="w-full p-2 text-gray-500 focus:outline-none" id="exercise-weight"
                     v-model="item.weight">
            </div>
          ...
          <button @click="addExercise" type="button"
        ...
        <div v-if="workoutType === 'cardio'" class="flex flex-col gap-y-4">
          <div class="flex flex-col gap-x-6 gap-y-2 relative md:flex-row" v-for="(item, index) in exercises"
            ...
            <div class="flex flex-col md:w-1/3">
              ...
              <select id="cardio-type" class="py-2 w-full text-gray-500 focus:outline-none" v-model="item.cardioType">
            ...
            <div class="flex flex-col flex-1">
              ...
              <input type="text" required class="w-full p-2 text-gray-500 focus:outline-none" id="distance"
                     v-model="item.distance">
            </div>
            <div class="flex flex-col flex-1">
              <label for="duration" class="mb-1 text-sm text-at-light-green">Duration</label>
              <input type="text" required class="w-full p-2 text-gray-500 focus:outline-none" id="duration"
                     v-model="item.duration">
            </div>
            <div class="flex flex-col flex-1">
              <label for="pace" class="mb-1 text-sm text-at-light-green">Pace</label>
              <input type="text" required class="w-full p-2 text-gray-500 focus:outline-none" id="pace"
                     v-model="item.pace">
            </div>
            <img @click="deleteExercise(item.id)" src="@/assets/images/trash-light-green.png"
                 class="h-4 w-auto absolute -left-5 cursor-pointer" alt="">
...
export default {
  ...
  setup() {
    ...
    const workoutType = ref('selected-workout');
