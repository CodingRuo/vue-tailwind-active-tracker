<template>
  <div v-if="dataLoaded" class="container mt-10 px-4">
    ...
    <div v-if="data.length === 0" class="w-full flex flex-col items-center">
      ...
      <router-link
          class="mt-6 py-2 px-6 rounded-sm text-sm text-white bg-at-green duration-200 border-solid border-2 border-transparent hover:border-at-light-green hover:bg-white hover:text-at-light-green"
          :to="{name: 'Create'}">Create Workout
      </router-link>
    </div>

    <!-- Data -->
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      <router-link :to="{name: 'View-Workout', params: {workoutId: workout.id}}" class="flex flex-col items-center bg-light-grey p-8 shadow-md cursor-pointer"
